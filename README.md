...
## Running testrepoassess in Quarkus dev mode
...
```
(testrepoassess should be included in the returned array)
...
[testrepoassess-schema.json](./src/main/resources/schemas/testrepoassess-schema.json ), run the following to create an
instance of the testrepoassess workflow:
```shell script
curl -v -X POST -H "Content-Type: application/json" http://localhost:8080/testrepoassess -d@input.json
```
